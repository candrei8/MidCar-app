...
function NuevoVehiculoContent() {
    ...
    const handleSave = async (formData: VehicleFormData) => {
        ...
        // Generate stock ID first (needed for image paths)
        ...
        if (formData.fotos && formData.fotos.length > 0) {
            for (let i = 0; i < formData.fotos.length; i++) {
                ...
                try {
                    const url = await uploadVehicleImage(foto.file, stockId, i)
                    const isPrincipal = formData.foto_principal === foto.id || (i === 0 && !formData.foto_principal)

                    imagenesArray.push({ url, es_principal: isPrincipal, orden: i })
        ...
        const newVehicle: Omit<Vehicle, 'id'> = {
            ...
            en_oferta: formData.en_oferta ?? false,
            ...
            fecha_itv_vencimiento: formData.fecha_itv_vencimiento || undefined,
            imagen_principal: imagenPrincipal,
            imagenes: imagenesArray as any,
